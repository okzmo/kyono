...
<svelte:head>
  <title>
    {userNotFound ? '404' : `${user.displayName} ${user.status ? '| ' + user.status : ''}`}
  </title>
  {#if !userNotFound}
...
<a
  href="/signin"
  class="fixed left-1/2 -translate-x-1/2 top-8 md:bottom-8 text-zinc-50/75 md:text-zinc-50/20 hover:text-zinc-50 active:text-zinc-50/70 transition-colors duration-75 h-fit focus-visible:outline-none focus-visible:text-zinc-50"
>
  <Kyodo height={22} width={28} />
</a>
